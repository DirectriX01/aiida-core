...
def identify_constraints(extras, pre_releases):
    """Identify dependency constraints

    For example:

        identify-constrainst all

    This command will analyze the current dependencies and compared them against
    the latest versions released on PyPI. It then lists all dependencies where the
    latest release is not compatible with the dependency specification. This function
    can thus be used to identify dependencies that might need to be bumped.
    """
